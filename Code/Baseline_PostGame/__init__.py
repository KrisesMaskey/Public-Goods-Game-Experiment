...
doc = """
Baseline Post Experiment Part
"""


class C(BaseConstants):
    NAME_IN_URL = 'Baseline_PostGame'
...
class Validation_Code_Page(Page):
    template_name = 'Baseline_PostGame/Validation_Code_Page.html'
    
    @staticmethod 
    def vars_for_template(player):
        game_earning = (float(player.participant.personal_account_balance) / 20) * 0.10
        pregame_earning = (float(player.participant.additional_question_MU) / 20) * 0.10
        total_earning = pregame_earning + game_earning + player.peq_bonus + 1.5 if player.participant.timeout_flag == False else 0
        ...
        return(dict(leave_flag = player.participant.leave_flag, timeout_flag = player.participant.timeout_flag, group_left_flag = player.session.vars['group_player_left'],
                    additional_question_MU = format(player.participant.additional_question_MU, '.2f'), additional_question_earning = format(pregame_earning, '.2f'), 
