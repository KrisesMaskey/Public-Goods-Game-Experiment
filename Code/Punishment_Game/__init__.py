...
doc = """
PGG Punishment Condition
"""


class C(BaseConstants):
    NAME_IN_URL = 'Punishment_Game'
    PLAYERS_PER_GROUP = 4
    NUM_ROUNDS = 40
...
class Player(BasePlayer):
    ...
    punish_B = models.BooleanField(initial=False)
    punish_C = models.BooleanField(initial=False)
    punish_D = models.BooleanField(initial=False)

    receive_punishment_from_B = models.BooleanField(initial=False)
    receive_punishment_from_C = models.BooleanField(initial=False)
    receive_punishment_from_D = models.BooleanField(initial=False)
    ...
    def after_all_players_arrive(group):
        ...
        if(len(players) > 1):
            for player in players:
                ...
                player.participant.p2_role = random.choice(['human', 'bot']) if player.session.config['randomize_roles'] else player.session.config['punishment_roles'][0]
                player.participant.p3_role = random.choice(['human', 'bot']) if player.session.config['randomize_roles'] else player.session.config['punishment_roles'][1]
                player.participant.p4_role = random.choice(['human', 'bot']) if player.session.config['randomize_roles'] else player.session.config['punishment_roles'][2]
        ...
        if(data["type"] == 'game-page'):            
            ...
            for i, label in enumerate(labels):
                if data["inputs"][i] == 'punish':
                    setattr(player, f'punish_{label}', True)         
...
class FeedbackPage(Page):
    ...
    def vars_for_template(player):
        ...
        contribution_list = [contribution_map[str(p_map[l])] for l in ["B", "C", "D"]]
        return(dict(p2_role = player.participant.p2_role, p3_role = player.participant.p3_role, p4_role = player.participant.p4_role,
                    player_contribution = player.round_contribution, contribution = contribution_list, condition = 'punish'))
...
class RoundWaitPage_2(WaitPage):
    ...
    def after_all_players_arrive(group):
        ...
        for player in players:
            ...
            for label in ['B', 'C', 'D']:
                if getattr(player, f'punish_{label}'):
                    ...
                    setattr(other_player, f'receive_punishment_from_{player_label}', True)
...
class RoundSummaryPage_2(Page):
    ...
    @staticmethod
    def vars_for_template(player):
        player_action_list = [["Punish", -4] if val else ["Do Nothing", 0] for val in [player.punish_B, player.punish_C, player.punish_D]]
        other_player_action_list = [["Punish", -12] if val else ["Do Nothing", 0] for val in [player.receive_punishment_from_B, player.receive_punishment_from_C, player.receive_punishment_from_D]]
...
page_sequence = [WaitingPage, GIExamplePage, RolePage, GameInterface, RoundWaitPage_1, RoundSummaryPage_1, PunishPage, FeedbackPage, RoundWaitPage_2, RoundSummaryPage_2]
