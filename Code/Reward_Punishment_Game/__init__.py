...
doc = """
Both Reward and Punishment Condition Game
"""


class C(BaseConstants):
    NAME_IN_URL = 'Reward_Punishment_Game'
    PLAYERS_PER_GROUP = 4
    NUM_ROUNDS = 40
...
class Player(BasePlayer):
    ...
    action_towards_B = models.StringField(initial="Do Nothing")
    action_towards_C = models.StringField(initial="Do Nothing")
    action_towards_D = models.StringField(initial="Do Nothing")

    action_from_B = models.StringField(initial="")
    action_from_C = models.StringField(initial="")
    action_from_D = models.StringField(initial="")
    ...
    def after_all_players_arrive(group):
        ...
        if(len(players) > 1):
            for player in players:
                ...
                player.participant.p2_role = random.choice(['human', 'bot']) if player.session.config['randomize_roles'] else player.session.config['reward_punishment_roles'][0]
                player.participant.p3_role = random.choice(['human', 'bot']) if player.session.config['randomize_roles'] else player.session.config['reward_punishment_roles'][1]
                player.participant.p4_role = random.choice(['human', 'bot']) if player.session.config['randomize_roles'] else player.session.config['reward_punishment_roles'][2]
    ...
    def live_method(player, data):
        if(data["type"] == 'game-page'):            
            ...
            for i, label in enumerate(labels):
                if data["inputs"][i] == 'punish':
                    setattr(player, f'action_towards_{label}', 'Punish')

                elif data["inputs"][i] == 'reward':
                    setattr(player, f'action_towards_{label}', 'Reward')
                
                else:
                    setattr(player, f'action_towards_{label}', 'Do Nothing')
...
class FeedbackPage(Page):
    ...
    def vars_for_template(player):
        ...
        contribution_list = [contribution_map[str(p_map[l])] for l in ["B", "C", "D"]]
        return(dict(p2_role = player.participant.p2_role, p3_role = player.participant.p3_role, p4_role = player.participant.p4_role,
                    player_contribution = player.round_contribution, contribution = contribution_list, condition = 'both'))
...
class RoundWaitPage_2(WaitPage):
    ...
    def after_all_players_arrive(group):
        ...
        for player in players:
            ...
            for label in ['B', 'C', 'D']:
                other_player_id = player_map[label]
                ...
                if getattr(player, f'action_towards_{label}') == 'Punish':
                    if player.participant.personal_account_balance > 4:
                        player.participant.personal_account_balance -= 4 
                    setattr(other_player, f'action_from_{player_label}', 'Punish')
                    if(other_player.participant.personal_account_balance > 12):
                        other_player.participant.personal_account_balance -= 12
                
                elif getattr(player, f'action_towards_{label}') == 'Reward':
                    if player.participant.personal_account_balance > 4:
                        player.participant.personal_account_balance -= 4 
                    setattr(other_player, f'action_from_{player_label}', 'Reward')
                    other_player.participant.personal_account_balance += 12   

                else:   
                    setattr(other_player, f'action_from_{player_label}', 'Do Nothing')        
...
class RoundSummaryPage_2(Page):
    ...
    @staticmethod
    def vars_for_template(player):
        player_action_value_map = {'Punish': -4, 'Reward': -4, 'Do Nothing': 0, '':0}
        other_player_action_value_map = {'Punish': -12, 'Reward': 12, 'Do Nothing': 0, '':0}

        player_action_list = [[val if val != '' else 'Do Nothing', player_action_value_map[val]] for val in [player.action_towards_B, player.action_towards_C, player.action_towards_D]]
        other_player_action_list = [[val if val != '' else 'Do Nothing', other_player_action_value_map[val]] for val in [player.action_from_B, player.action_from_C, player.action_from_D]]
        ...
        sign = '+' if other_player_action_payoff > 0 else '-'
        ...
        return(dict(p2_role = player.participant.p2_role, p3_role = player.participant.p3_role, p4_role = player.participant.p4_role, 
                    ...
                    other_player_action = other_player_action_list, payoff_1 = format(abs(player_action_payoff), '.2f'), 
                    payoff_2_sign = sign, payoff_2 = format(abs(other_player_action_payoff), '.2f')))


page_sequence = [WaitingPage, GIExamplePage, RolePage, GameInterface, RoundWaitPage_1, RoundSummaryPage_1, DecisionPage, FeedbackPage, RoundWaitPage_2, RoundSummaryPage_2]
